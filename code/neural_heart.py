...
frequency = 500 # sampling rate
nosOfTestSamples = 1000
...
def build_cnn_mnist(input_var=None):
    ...
    # convolutions are supported as well; see the docstring.
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=10, filter_size=(1, 5),
            nonlinearity=lasagne.nonlinearities.rectify,
            W=lasagne.init.GlorotUniform())
    # Expert note: Lasagne provides alternative convolutional layers that
    # override Theano's choice of which implementation to use; for details
...
#    return predicted_gene_expres, y_val
    ...
    np.savez('heart_model.npz', *lasagne.layers.get_all_param_values(network))
...
def prediction(model='cnn_mnist'):
    ...
    with np.load('heart_model.npz') as f:
        param_values = [f['arr_%d' % i] for i in range(len(f.files))]
    lasagne.layers.set_all_param_values(network, param_values)
